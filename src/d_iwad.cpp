...
static const iwad_t iwads[] =
{
    ...
    { "doom2f.wad",   GameMission_t::doom2,     GameMode_t::commercial, "Doom II: L'Enfer sur Terre" },
    ...
    { "rekkrsa.wad",  GameMission_t::doom,      GameMode_t::retail,     "REKKR" }, // [crispy] REKKR
    { "rekkrsl.wad",  GameMission_t::doom,      GameMode_t::retail,     "REKKR: Sunken Land" }, // [crispy] REKKR: Sunken Land (Steam retail)
...
static char *GetRegistryString(registry_value_t *reg_val)
{
     ...
     && valtype == REG_SZ)
    {
        ...
        result = (decltype(        result)) malloc(len + 1);
...
static char *SearchDirectoryForIWAD(const char *dir, int mask, GameMission_t *mission)
{
    ...
    for (size_t i=0; i<arrlen(iwads); ++i) 
    {
        auto iwad_mission = static_cast<std::underlying_type_t<GameMission_t> >( iwads[i].mission );
        if (( ( 1 <<  iwad_mission) & mask) == 0)
        {
            continue;
        }
...
const iwad_t **D_FindAllIWADs(int mask)
{
    ...
    result =  create_structure<const iwad_t*[]>( arrlen(iwads) + 1 );
    result_len = 0;
