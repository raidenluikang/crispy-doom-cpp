...
void P_SetPsprite(player_t * player, int position, statenum_t stnum)
{
    ...
    do
    {
        ...
        if (state->action)
        {                       // Call action routine.
            state->action(nullptr, player, psp);
            if (!psp->state)
            {
                break;
            }
...
void P_PostMorphWeapon(player_t * player, weapontype_t weapon)
{
    ...
    P_SetPsprite(player, ps_weapon,
                 statenum_t{ WeaponInfo[weapon][player->mclass].upstate } );
}
...
void P_BringUpWeapon(player_t * player)
{
    ...
    if (player->mclass == PCLASS_FIGHTER && player->pendingweapon == WP_SECOND
        && player->mana[MANA_1])
    {
        new_ = S_FAXEUP_G;
    }
    ...
    {
        new_ = statenum_t{ WeaponInfo[player->pendingweapon][player->mclass].upstate };
    }
    player->pendingweapon = WP_NOCHANGE;
    player->psprites[ps_weapon].sy = WEAPONBOTTOM;
...
boolean P_CheckMana(player_t * player)
{
    ...
    mana = WeaponInfo[player->readyweapon][player->mclass].mana;
    count = WeaponManaUse[player->mclass][player->readyweapon];
    if (mana == MANA_BOTH)
    {
    ...
    P_SetPsprite(player, ps_weapon,
                 statenum_t{ WeaponInfo[player->readyweapon][player->mclass].downstate } );
    return (false);
}
...
void P_FireWeapon(player_t * player)
{
    ...
    P_SetMobjState(player->mo, statenum_t{ PStateAttack[player->mclass] } );    // S_PLAY_ATK1);
    ...
    {
        attackState = statenum_t{ player->refire ?
            WeaponInfo[player->readyweapon][player->mclass].holdatkstate
            : WeaponInfo[player->readyweapon][player->mclass].atkstate
            };
    }
...
void A_WeaponReady(mobj_t *actor, player_t *player, pspdef_t *psp)
{
    ...
    if (player->mo->state >= &states[PStateAttack[player->mclass]]
        && player->mo->state <= &states[PStateAttackEnd[player->mclass]])
    {
        P_SetMobjState(player->mo, PStateNormal[player->mclass]);
    }
    ...
    {
        P_SetPsprite(player, ps_weapon,
                     WeaponInfo[player->readyweapon][player->mclass].
                     downstate);
        return;
    }
...
void A_FHammerAttack(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
  ...
  hammerdone:
    if (player->mana[MANA_2] <
        WeaponManaUse[player->mclass][player->readyweapon])
...
void A_FHammerThrow(mobj_t *mobj, player_t * player, pspdef_t * psp)
{
    ...
    player->mana[MANA_2] -= WeaponManaUse[player->mclass][player->readyweapon];
...
void A_FSwordAttack(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
    ...
    player->mana[MANA_1] -= WeaponManaUse[player->mclass][player->readyweapon];
    player->mana[MANA_2] -= WeaponManaUse[player->mclass][player->readyweapon];
    pmo = player->mo;
