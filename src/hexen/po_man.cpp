                      ...
                      overRide)
{
    ...
    pe = zmalloc<decltype(    pe)>(sizeof(polyevent_t), PU_LEVSPEC, 0);
...
boolean EV_OpenPolyDoor(line_t * line, byte * args, podoortype_t type)
{
    ...
    pd = zmalloc<decltype(    pd)>(sizeof(polydoor_t), PU_LEVSPEC, 0);
        ...
        for (i = po->bbox[BOXLEFT]; i <= po->bbox[BOXRIGHT]; i++)
        {
                ...
                && j < bmapheight * bmapwidth)
            {
                ...
                {               // Create a new link at the current block cell
                    *link = zmalloc<decltype(                    *link)>(sizeof(polyblock_t), PU_LEVEL, 0);
                    (*link)->next = nullptr;
                    (*link)->prev = nullptr;
                    (*link)->polyobj = po;
                    continue;
                ...
                {
                    tempLink->next = zmalloc<decltype(                    tempLink->next)>(sizeof(polyblock_t),
                                              PU_LEVEL, 0);
...
static void InitBlockMap(void)
{
    ...
    PolyBlockMap = zmalloc<decltype(    PolyBlockMap)>(bmapwidth * bmapheight * sizeof(polyblock_t *),
                            PU_LEVEL, 0);
...
static void SpawnPolyobj(int index, int tag, boolean crush)
{
    ...
    for (i = 0; i < numsegs; i++)
    {
            ...
            segs[i].linedef->arg1 == tag)
        {
            ...
            polyobjs[index].segs = zmalloc<decltype(            polyobjs[index].segs)>(PolySegCount * sizeof(seg_t *),
                                            PU_LEVEL, 0);
    ...
    {                           // didn't find a polyobj through PO_LINE_START
        ...
        if (polyobjs[index].numsegs)
        {
            ...
            polyobjs[index].ry = 0; // [crispy]
            polyobjs[index].rtheta = 0; // [crispy]
            polyobjs[index].segs = zmalloc<decltype(            polyobjs[index].segs)>(polyobjs[index].numsegs
...
static void TranslateToStartSpot(int tag, int originX, int originY)
{
    ...
    po->originalPts = zmalloc<decltype(    po->originalPts)>(po->numsegs * sizeof(vertex_t), PU_LEVEL, 0);
    po->prevPts = zmalloc<decltype(    po->prevPts)>(po->numsegs * sizeof(vertex_t), PU_LEVEL, 0);
